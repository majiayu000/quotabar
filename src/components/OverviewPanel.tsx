  ...
  return <section className="analysis-section workspace-trend"><header><div><h2>用量趋势</h2><p>{report.since === report.until && metric === 'tokens' ? `按小时查看 · ${report.timezone}` : '按日查看 · 点击柱形查看当天来源与会话'}</p></div><div className="analysis-ranges" aria-label="图表指标"><button aria-pressed={metric === 'tokens'} onClick={() => setMetric('tokens')}>Token</button><button aria-pressed={metric === 'cost'} onClick={() => setMetric('cost')}>费用</button></div></header>
    {rows.length === 0 ? <p className="analysis-empty">当前周期没有历史记录。</p> : <>
      {report.since === report.until && metric === 'tokens' ? <HourlyPlot report={report} /> : <div className="workspace-plot"><div className="workspace-axis">{[1, .75, .5, .25, 0].map((scale) => <span key={scale}>{metric === 'tokens' ? compactNumber(max * scale) : money(max * scale)}</span>)}</div><div className="analysis-chart" role="group" aria-label="每日用量趋势">{dates.map(({ date, points }, index) => {
        ...
        const costLabel = knownCost.length === 0 ? '费用未知' : `${missingCost ? '≥ ' : ''}${money(total)} USD 费用参考`;
        const label = `${date} · ${metric === 'tokens' ? number(total) + ' Tokens' : costLabel}${missingCost && knownCost.length > 0 ? ' · 价格不完整' : ''}`;
...
export function AnalysisApp({ visible = true, providerContent, providerView, theme: workspaceTheme, summaries = [], quotaWindows = [], onProviderView, onThemeChange, onRefreshProvider }: {
  ...
  return <div className={`analysis-app ${dark ? 'analysis-dark' : ''}`}>
    <aside className="analysis-sidebar">
      ...
      <nav aria-label="分析导航">{ANALYSIS_VIEWS.filter(([value]) => value !== 'sources' && value !== 'settings').map(([value, label], index) => <button key={value} title={label} aria-current={view === value ? 'page' : undefined} onClick={() => navigate(value)}><WorkspaceIcon name={value} />{label}<kbd>{index + 1}</kbd></button>)}</nav>
      <div className="workspace-source-nav"><small className="analysis-nav-label">记录来源 <span>{readySources.length}</span></small><button aria-pressed={source === 'all'} onClick={() => { chooseSource('all'); navigate('overview'); }}><WorkspaceIcon name="sources" />全部来源</button>{readySources.map((item) => <button key={item.name} aria-pressed={source === item.name} onClick={() => { chooseSource(item.name); navigate('overview'); }}><span className="workspace-source-glyph"><SourceIcon source={item.name} /></span>{item.display_name}</button>)}</div>
      <div className="analysis-management"><nav aria-label="管理导航">{ANALYSIS_VIEWS.filter(([value]) => value === 'sources' || value === 'settings').map(([value, label]) => <button key={value} title={label} aria-current={view === value ? 'page' : undefined} onClick={() => navigate(value)}><WorkspaceIcon name={value} />{label}</button>)}</nav><div className="analysis-local"><WorkspaceIcon name="shield" /><span>本机工作区<small>自动读取本地记录</small></span></div></div>
    </aside>
    <main><header className="workspace-topbar"><div className="workspace-location"><WorkspaceIcon name={view} /><h1>{title}</h1></div><div><button aria-label="打开菜单栏面板" title="打开菜单栏面板" onClick={() => void backend.openQuotaPopover().catch((reason) => setActionError(`无法打开菜单栏面板：${String(reason)}`))}><WorkspaceIcon name="quota" /><span>菜单栏</span></button><button aria-label="切换深浅主题" title={dark ? '切换浅色外观' : '切换深色外观'} onClick={changeTheme}><WorkspaceIcon name="theme" /></button>{!providerPage && view !== 'sources' && <button disabled={!report} onClick={() => { setSavedExport(null); setShareOpen(true); }}><WorkspaceIcon name="share" />导出摘要</button>}</div></header>
    <div className="analysis-content" ref={contentRef}>
    {view === 'overview' && <div className="workspace-page-heading"><div><h2>用量与费用</h2></div><p>{report ? `${report.since} — ${report.until}` : rangeLabel}<br />{sourceLabel}</p></div>}
      ...
      {providerPage ? null : view === 'sources' ? <section className="analysis-section"><header><h2>本地记录与连接状态</h2><button onClick={() => setRefresh((value) => value + 1)}>重新检测来源</button></header>{!catalog && !catalogError && <p role="status">正在发现来源…</p>}{catalog?.diagnostics.map((item) => <article className="analysis-source-row" key={item.name}><div><h3>{item.display_name}</h3><p>{item.detail}</p><small>{item.setup}</small></div><span className={`analysis-status ${item.status}`}>{item.status === 'missing' ? '未发现' : item.status === 'configured' ? '已配置' : '已发现'}</span></article>)}</section> : <>
        ...
        {report && <>
          ...
          {view === 'usage' ? <><div className="analysis-list-toolbar"><div className="analysis-ranges" aria-label="用量分组"><button aria-pressed={grouping === 'session'} onClick={() => { setGrouping('session'); setSearch(''); }}>会话</button><button aria-pressed={grouping === 'project'} onClick={() => { setGrouping('project'); setSearch(''); }}>项目与会话</button><button aria-pressed={grouping === 'model'} onClick={() => { setGrouping('model'); setSearch(''); }}>模型</button></div><input aria-label="搜索用量" placeholder="搜索名称或来源…" value={search} onChange={(event) => setSearch(event.target.value)} /></div>{grouping === 'session' ? <SessionLedger key={key} report={report} search={search} /> : grouping === 'project' ? <AnalysisProjects key={key} report={report} catalog={catalog} search={search} /> : <AnalysisModels report={report} search={search} />}<details className="workspace-supplement"><summary>查看 Token 构成</summary><TokenComposition report={report} /></details></> : <>
            ...
            {view === 'history' && <><PeriodComparison key={key} report={report} source={source!} query={query} /><UsageActivity report={report} onDate={(date) => setSelectedDate(date)} /><details className="workspace-supplement"><summary>查看 Token 构成</summary><TokenComposition report={report} /></details><SessionLedger key={key} report={report} /></>}
      ...
      {(connectedCount === 0 || setupOpen) && <details className="setup-services" open={connectedCount === 0 || setupOpen ? true : undefined}>
        ...
        {summaries.map((summary) => (
          <details className="setup-service" key={summary.id}>
            ...
            {summary.lastSuccessAt != null && <p>最近成功读取 {formatEventTime(new Date(summary.lastSuccessAt).toISOString())}</p>}
          ...
          {mostConstrained.length > 0 ? mostConstrained.map((window, index) => {
            ...
            >
              ...
              <div className="overview-timing">
                {summary?.lastSuccessAt != null && <span>最近成功读取 {new Date(summary.lastSuccessAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}</span>}
                {window.resetLabel && <span>{window.resetLabel} 后重置</span>}
              </div>
            </button>
          ); }) : (
