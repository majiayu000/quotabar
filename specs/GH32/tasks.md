...
- [ ] `SP32-T2` Owner: codex. Dependencies: `SP32-T1`. Covers: `B-001`, `B-002`, `B-003`. Done when: Rust `CodexStats` model、`get_codex_stats` command/handler、`fetch_codex_stats()`、`HistoryStatsCache`、JSONL stats helpers 和专属 imports 已删除，其他 Codex APIs 未改语义。 Verify: `! rg -n "get_codex_stats|CodexStats|HistoryStatsCache|fetch_codex_stats" src-tauri/src && cargo check --manifest-path src-tauri/Cargo.toml`.
- [ ] `SP32-T3` Owner: codex. Dependencies: `SP32-T1`, `SP32-T2`. Covers: `B-002`, `B-004`. Done when: 全仓目标标识符负向检查无结果，且 diff 不包含 issue 非目标文件。 Verify: `! rg -n "getCodexStats|get_codex_stats|CodexStats|HistoryStatsCache|fetch_codex_stats" src src-tauri/src tests && ! git diff --name-only origin/main...HEAD | rg '^(src-tauri/src/services/(cost|codex_pricing)\.rs|specs/GH10/)'`.
