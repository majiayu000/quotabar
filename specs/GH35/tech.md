...
- throwing storage 表驱动覆盖全部 9 个 service 写入口与 App-owned saver。
...
App wiring 通过两类证据约束：storage subscriber 的可执行行为测试；以及多个独立 `rg -q` 断言确认 App 注册/cleanup subscriber，并分别出现 `saveActiveTab(tab)`、`saveTheme(newTheme)`、`saveDockHidden(newValue)`、`saveSettingsExpanded(opening)`、`saveSettingsExpanded(false)` 的真实调用，app_state 初始化 failure check 也必须存在。任一遗漏或搜索错误均失败。
...
| `B-002` 设置失败提示准确且本会话值稳定 | `App.tsx`, storage subscriber, app_state/service savers, adapter shadow | subscriber 行为测试；budget/tab reread；App-owned success/failure matrix；分离 wiring 断言 |
