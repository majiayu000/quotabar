...
- [ ] `SP35-T2` Owner: codex. Dependencies: `SP35-T1`. Covers: `B-001`, `B-002`, `B-005`, `B-006`. Done when: budget、notification settings、panel sections、switcher、tray cycle/style/visibility 与 app_state saver 全部返回 adapter boolean；用户设置启用 shadow+subscriber，现有 getters 通过 adapter 读取 shadow且保留原 catch/default；成功/throwing-storage 矩阵覆盖所有入口、精确 key/format，并证明 budget/tab reread 保持失败写入的新值。 Verify: `npx vitest run tests/storage_write_failures.test.ts tests/budget.test.ts tests/notifications.test.ts tests/panel_sections.test.ts tests/switcher_providers.test.ts tests/tray_style.test.ts`.
- [ ] `SP35-T3` Owner: codex. Dependencies: `SP35-T2`. Covers: `B-002`, `B-005`. Done when: App 注册并 cleanup 单一 storage failure subscriber；tab/theme/dock/settings handlers 调用对应 app_state saver，其他用户 setting saver 自动触发同一 channel；`app_state.ts` 初始化修复写入显式检查失败。 Verify: `npm run build && rg -q "STORAGE_WRITE_FAILURE_MESSAGE" src/App.tsx && rg -q "showStorageWriteFailure" src/App.tsx && rg -q "subscribeStorageWriteFailures\\(showStorageWriteFailure\\)" src/App.tsx && rg -q "saveActiveTab\\(tab\\)" src/App.tsx && rg -q "saveTheme\\(newTheme\\)" src/App.tsx && rg -q "saveDockHidden\\(newValue\\)" src/App.tsx && rg -q "saveSettingsExpanded\\(opening\\)" src/App.tsx && rg -q "saveSettingsExpanded\\(false\\)" src/App.tsx && rg -q "if \\(!saveTrayEnabled\\('claude', true\\)\\)" src/services/app_state.ts && npx vitest run tests/storage_write_failures.test.ts`.
- [ ] `SP35-T4` Owner: codex. Dependencies: `SP35-T1`, `SP35-T2`. Covers: `B-003`, `B-004`. Done when: dedupe 写失败时 `shouldNotify` 返回 `false` 且不保留 shadow，storage 恢复后重试返回 `true`；event log 写失败仍返回并可读回本会话事件，adapter 产生 error 级证据且不触发用户 toast。 Verify: `npx vitest run tests/storage_write_failures.test.ts tests/notifications.test.ts tests/event_log.test.ts`.
