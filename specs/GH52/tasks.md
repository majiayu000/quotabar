...
- [ ] `SP52-T2` Owner: codex. Dependencies: T1. Covers: `B-001`~`B-006`. Done when: Claude/Codex/Cursor/Antigravity 全部接入 owner-shared guard；Codex bundle atomic；stale callbacks/errors/loading suppressed；Antigravity interval 0 pauses。 Verify: provider deferred real-effect matrix。
- [ ] `SP52-T3` Owner: codex. Dependencies: T1. Covers: `B-001`~`B-005`. Done when: Cost overview/daily 独立 generations；同 lane latest-wins；cleanup invalidates；loading/error/daily regressions exact。 Verify: cost cross-lane deferred tests。
- [ ] `SP52-T4` Owner: codex. Dependencies: T1-T3. Covers: `B-007`. Done when: dev-only renderer 与 resolved React exact version；11-path allowlist；overall diff ≥80%；coordinator critical 100%；full frontend/build/Rust pass。 Verify: tech Test Plan。
...
- Spec PR 仅三份 GH52 文档；implementation 才修改 11-path allowlist 文件。
