...
- Claude/Cursor/Antigravity 的 old-success、old-failure、old-finally 与 unmount paths 均有 deterministic evidence。
- Codex 任一 bundle member rejection 仅在 generation current 时写 failure；旧 bundle 的任何 terminal path 不产生 partial commit。
- Cost overview 与 daily 可并发且互不 invalidate；每条 lane 内 old result 不能覆盖 new result。
...
- implementation 仅含 tech spec 11-path allowlist；新增 executable TS/TSX ≥80%，request coordinator 与 stale terminal critical paths 100%。
