...
保持 `usePopoverWindow(containerRef, resizeDeps): boolean` public contract 不变。Tauri visibility effect 内使用三个 effect-owned bindings：
...
### 5. Direct callback guard source gate

React 19 renderer 在 component unmount 后可能忽略 setter 而不产生 render，因此 runtime “零 render”不能单独证明 `B-005` 的零 state write。`tests/popover_window_visibility.test.tsx` 必须同时使用 TypeScript AST 对真实 `src/hooks/use_popover_window.ts` 做 fail-closed source validation：

- 绑定 Tauri visibility effect 内唯一直接 `appWindow.onFocusChanged` registration 及其真实 inline callback；禁止用 dummy callback/dead function 满足 gate。
- callback 必须是 block arrow；第一条 executable statement 必须 exact 为 `if (!mounted) return;` 或单-return block，无 else、无额外 condition/token。
- mounted guard 之后才允许 `read_superseded = true` 与 `setWindowVisible(focused)`；这两条各一次、顺序固定，callback 内不得有额外 setter。
- validator 对 missing guard、guard after setter、wrong mounted identifier、non-return guard、else branch、dummy/dead callback、wrong setter payload 全部失败。
- 同一测试文件使用真实 source success case 与上述 in-memory mutation fixtures 证明 validator fail closed；禁止仅用 regex/count 或只断言 helper。

该 source gate 属于测试代码，不新增 scripts/dependencies/allowlist path；targeted 与 full frontend test 都执行它。
...
| React suppresses unmounted state writes | real callback AST guard gate + adversarial mutation fixtures。 |
...
| `B-005` | cleanup before each read/subscription terminal + captured callback + real callback AST guard gate |
