...
App replacement gate 通过 `git show origin/main:src/App.tsx` 读取 baseline，要求旧 `import './styles.css';` 恰好出现一次，只将该 byte sequence 替换为三个 direct imports，并断言当前 `App.tsx` bytes 与 computed expected 完全相等。该门禁证明 App diff 没有 executable code、双引号 import、formatting 或其他混入；随后再提取所有 top-level `.css` imports，数组必须与五项 expected 完全相等。
...
本变更不新增可执行 TS/TSX 行：App exact replacement gate 证明 `App.tsx` 只用三个 CSS import declaration 替换一个 CSS import，V8 LCOV 不把这些 declaration 计为 executable line。GH35 diff coverage checker 在“零 measurable added lines”时会正确 fail closed，因此不适用于本次纯 CSS/import mechanical split。禁止加入 dummy runtime code 伪造 coverage；改由 exact App bytes/import array、source hash 与 production bundle hash 门禁覆盖全部新增/移动 bytes。
...
| App allowlist 混入 executable/formatting edit | 从 `origin/main:src/App.tsx` computed exact replacement；current bytes 必须完全相等。 |
...
| `B-003` cascade order | App direct imports | origin/main exact replacement bytes + exact CSS import array |
