...
- [ ] `SP45-T1` Owner: codex. Dependencies: merged GH45 spec PR. Covers: `B-006`. Done when: fresh implementation branch 上 edit 前确认 `src/redesign.css` 为 861 行/15,665 bytes/source SHA exact，App CSS imports 与 baseline exact，baseline build 为一个 52,095-byte/bundle SHA exact asset；任一漂移则停止并更新 spec。 Verify: 执行 tech spec Preflight Contract。
...
- [ ] `SP45-T3` Owner: codex. Dependencies: `SP45-T2`. Covers: `B-003`, `B-004`. Done when: current App 精确等于 `origin/main:src/App.tsx` 仅做唯一 redesign import 1→2 replacement 后的 computed expected；六项 CSS import order exact；没有 `@import`；production bundle count/size/SHA exact。 Verify: 执行 exact App/import/bundle gates。
