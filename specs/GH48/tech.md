...
9. fresh read failure 时不做 persistent overwrite；session map 已保证本 session dedupe，返回 sent。write 使用 `preserveSessionValue: true, notifyUser: false, logErrorDetails: false`；false 时 shadow 保存 merged state，仍返回 sent，且 notification path 只 console.error 固定 `Failed to persist local setting.`。
...
`StorageWriteOptions` 新增 optional `logErrorDetails?: boolean`，default/omitted 保持现有 detailed storage logging；仅 notification dedupe commit 传 `false`。adapter 此时仍保留 session shadow 和 boolean outcome，但 console 只接收固定字符串，不接收 caught `Error`。这是 notification callsite 的最小安全扩展，不改变其他 storage caller。
...
| notification or storage original errors leak | fixed delivery/callback constants + notification-only `logErrorDetails: false` + exact console assertions。 |
...
| `B-005` post-send persistence failure | fresh-read failure/write failure both sent + session duplicate；write console exact fixed string且无 Error arg |
