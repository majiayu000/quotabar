...
`src/services/storage.ts` 的 `readStorageItem` 返回 `string | null` 并允许 access error 抛给 service；各 service 用 12 个 catch 返回默认/空值，其中 6 个为空 catch。该结构无法区分 missing、访问失败与 decode failure。`loadNotified()` 将读取失败变成 `{}`，因此在写入仍可用时 `shouldNotify()` 会 fail open。
...
4. access error 返回 `failure`，`console.error` 使用固定文案与原 access error，但不记录 key/value。
...
App 新增 `showStorageReadFailure` callback，复用现有 toast timeout；独立 effect 必须 `return subscribeStorageReadFailures(showStorageReadFailure);`。因为 state initializer 早于 effect，pending 机制保证 startup failure 不丢失。
...
- notification/panel/switcher：根必须是非数组 object；known field 缺失沿用 default，存在则必须是 boolean；未知字段忽略。
...
- adapter：shadow/value/missing/access/decode 三态、safe log、active subscriber、unsubscribe、listener error。
- startup：在无 subscriber 时触发多个 user-visible failure，订阅后 listener 恰好一次；再次订阅不会重复历史 pending。
...
| startup failure 发生在 effect 前而丢 toast | pending boolean + subscribe-time delivery；多失败 coalescing 测试。 |
| decoder 过宽继续吞坏数据 | known fields strict validation；每类 schema malformed case。 |
| decoder error 泄漏 raw value | corruption log 固定文案，不传原 decoder error/key/value。 |
...
| `B-004` startup coalescing | pending + App effect | multi-failure then subscribe exactly once；cleanup exact assertion |
...
rg -q 'return subscribeStorageReadFailures\(showStorageReadFailure\);' src/App.tsx
npm test
npm run build
