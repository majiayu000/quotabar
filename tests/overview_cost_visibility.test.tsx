...
describe('Analysis window', () => {
  it('shows real model rows for sources without project support and preserves raw token totals', async () => {
    ...
    await act(async () => renderer.root.findAllByType('button').find((button) => button.children.includes('Usage details'))!.props.onClick());
  ...
  it('navigates quota and settings inside the workspace while retaining the provider owner', async () => {
    ...
    await act(async () => click('Subscription quota'));
    expect(renderer.root.findByType('h1').children).toEqual(['Subscription quota']);
  ...
  it('ignores older range results and never starts provider polling', async () => {
    ...
    let renderer!: ReactTestRenderer;
    await act(async () => { renderer = create(createElement(AnalysisApp)); });
    await act(async () => { renderer.root.findAllByType('button').find((button) => button.children.includes('Today'))!.props.onClick(); });
  ...
  it('opens real recent sessions, filters projects, and preserves missing cache values', async () => {
    ...
    const detail = renderer.root.findByProps({ 'aria-label': 'Session details' });
    ...
    await act(async () => renderer.root.findByProps({ 'aria-label': 'Close session details' }).props.onClick());
    await act(async () => click('All sessions ↗'));
    expect(renderer.root.findByProps({ 'aria-label': 'Session sort order' })).toBeDefined();
    await act(async () => renderer.root.findByProps({ 'aria-label': 'Session sort order' }).props.onChange({ target: { value: 'tokens' } }));
    ...
    await act(async () => renderer.root.findByProps({ 'aria-label': 'Filter session projects' }).props.onChange({ target: { value: '/work/one' } }));
  ...
  it('saves a private summary through native IPC and reports failed writes', async () => {
    ...
    let renderer!: ReactTestRenderer;
    await act(async () => { renderer = create(createElement(AnalysisApp)); });
    await act(async () => renderer.root.findAllByType('button').find((button) => button.children.includes('Export summary'))!.props.onClick());
    ...
    await act(async () => { renderer.root.findByProps({ className: 'workspace-primary' }).props.onClick(); await Promise.resolve(); });
    expect(JSON.stringify(renderer.root.findByProps({ role: 'alert' }).children)).toContain('Could not save summary: Error: Disk full');
    await act(async () => renderer.unmount());
  });
  ...
  it('persists names across remounts, isolates sources, restores source metadata', async () => {
    ...
    await act(async () => renderer.root.findAllByType('button').find((button) => button.children.includes('Rename'))!.props.onClick());
    ...
    await act(async () => renderer.root.findAllByType('button').find((button) => button.children.includes('Restore source name'))!.props.onClick());
...
describe('workspace quota freshness', () => {
  it('shows the most constrained window and the stale-data error beside it', async () => {
    ...
    expect(JSON.stringify(card.findByProps({ role: 'alert' }).children)).toContain('Showing stale data');
    expect(JSON.stringify(card.findByProps({ role: 'alert' }).children)).toContain('Could not read quota');
    expect(JSON.stringify(card.findByProps({ role: 'alert' }).children)).not.toContain('Service unavailable');
    expect(card.findAllByType('small').some(node => node.children.join('').includes('Last successful read'))).toBe(true);
    await act(async () => renderer.unmount());
  });
});
...
describe('coherent workspace queries', () => {
  it('applies exact model and project filters without changing the page and reuses the report across views', async () => {
    ...
    let renderer!: ReactTestRenderer;
    await act(async () => { renderer = create(createElement(AnalysisApp)); });
    await act(async () => renderer.root.findByProps({ 'aria-label': 'Filter models' }).props.onChange({ target: { value: 'gpt-5' } }));
    ...
    expect(renderer.root.findByType('h1').children).toEqual(['Quota usage']);
    expect(JSON.stringify(renderer.toJSON())).toContain('456');
    await act(async () => renderer.root.findByProps({ 'aria-label': 'Filter projects' }).props.onChange({ target: { value: '/work/app' } }));
    ...
    expect(renderer.root.findByProps({ 'aria-label': 'Filter projects' }).props.value).toBe('/work/app');
    await act(async () => renderer.unmount());
  });
  ...
  it('keeps same-scope data during refresh and labels it stale on failure', async () => {
    ...
    expect(JSON.stringify(renderer.toJSON())).toContain('Showing previous results');
    await act(async () => reject(new Error('Unreadable records')));
    expect(JSON.stringify(renderer.toJSON())).toContain('321');
    expect(JSON.stringify(renderer.toJSON())).toContain('Refresh failed; showing stale data.');
...
describe('startup snapshot display', () => {
  it('paints a saved report before fresh aggregation completes, then replaces it', async () => {
    ...
    expect(JSON.stringify(renderer.toJSON())).not.toContain('Showing previous results');
    await act(async()=>renderer.unmount());
  });
  ...
  it('shows a loading skeleton without invented indexing progress when no saved report exists', async () => {
    ...
    expect(JSON.stringify(renderer.toJSON())).not.toContain('Building the initial index');
    await act(async()=>renderer.unmount());
  });
...
describe('history cost completeness presentation', () => {
  it('distinguishes unknown, actual zero and a partially priced total', async () => {
    ...
    let renderer!: ReactTestRenderer;
    await act(async () => { renderer = create(createElement(AnalysisApp)); });
    await act(async () => renderer.root.findAllByType('button').find((button) => button.children.includes('Cost'))!.props.onClick());
    const labels = renderer.root.findByProps({ 'aria-label': 'Daily usage trend' }).findAllByType('button').map((button) => button.props['aria-label']);
    expect(labels[0]).toBe('2026-09-01 · Unknown cost');
    expect(labels[1]).toBe('2026-09-02 · $0.00 USD cost reference');
