...
async function open() { await act(async () => byLabel(`全部服务（${summaries.length}）`).props.onClick()); }

it('bounds navigation and highlights All when the current service is not a favorite', async () => {
  await mount('grok');
  expect(navButtons()).toHaveLength(5);
  expect(byLabel(`全部服务（${summaries.length}）`).props['aria-current']).toBe('page');
  await open();
...
it('searches case-insensitively, reports no matches, and selects a non-favorite', async () => {
  ...
  await act(async () => byLabel('搜索服务').props.onChange({ target: { value: ' GROK ' } }));
  ...
  await open();
  await act(async () => byLabel('搜索服务').props.onChange({ target: { value: 'no-match' } }));
  expect(renderer!.root.findByProps({ className: 'provider-picker-empty' }).children).toEqual(['没有匹配的服务']);
});

it('requires replacing a favorite at the limit and persists the chosen order', async () => {
  await mount(); await open();
  await act(async () => byLabel('收藏 Grok').props.onClick());
  expect(renderer!.root.findByProps({ role: 'status' }).children.join('')).toContain('请先取消');
  await act(async () => byLabel('取消收藏 Claude').props.onClick());
  await act(async () => byLabel('收藏 Grok').props.onClick());
...
it('preserves an intentionally empty favorite list and excludes hidden providers', async () => {
  ...
  await act(async () => byLabel('全部服务（4）').props.onClick());
  expect(renderer!.root.findAllByProps({ 'aria-label': '收藏 Grok' })).toHaveLength(0);
});
...
it('reports invalid stored preferences and failed writes without losing the current choice', async () => {
  ...
  await act(async () => byLabel('收藏 Codex').props.onClick());
  expect(navButtons()).toHaveLength(3);
  expect(renderer!.root.findByProps({ role: 'status' }).children.join('')).toContain('未能保存');
...
it('keeps thirty providers in the searchable list while the top bar stays at five buttons', async () => {
  ...
  expect(navButtons()).toHaveLength(5);
  await act(async () => byLabel('全部服务（30）').props.onClick());
  expect(renderer!.root.findAllByProps({ className: 'provider-picker-row' })).toHaveLength(30);
  await act(async () => byLabel('搜索服务').props.onChange({ target: { value: 'Agent 30' } }));
